...
void calculate_lengths(size_t numVectors, size_t dimVectors, const int *vectors, double *lengths, int blockSize,
    ...
    for (size_t blocks = 0; blocks < numBlocks; blocks += maxBlockCount) {
        ...
        size_t currNumVectors = std::min(currNumBlocks * blockSize, numVectors - vectorsProcessed);
        ...
        cudaMemcpy(dVectors, vectors + elementsProcessed, currNumElements * sizeof(int), cudaMemcpyHostToDevice);

        calculate_lengths_kernel<<<currNumBlocks, blockSize>>>(currNumVectors, dimVectors, dVectors, dLengths);

        cudaMemcpy(lengths + vectorsProcessed, dLengths, currNumVectors * sizeof(double), cudaMemcpyDeviceToHost);
